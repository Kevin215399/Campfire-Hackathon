...
public class LevelGenerator : MonoBehaviour
{
    ...
    private void GenerateRoom(int x, int y)
    {
        ...
        else
        {
            ...
            while (!works)
            {
                current.roomType = Random.Range(0, 13 - (endCreated?1:0));
                ...
                if (intersectionRooms >= 3 && current.roomType == 0 && Random.Range(0, 7) != 0)
                    continue;
                ...
                {
                    case 0://all
                        if (leftRoom != -1 && !roomOpenings[rooms[leftRoom].roomType].right)
                            works = false;
                        if (rightRoom != -1 && !roomOpenings[rooms[rightRoom].roomType].left)
                            works = false;
                        if (upRoom != -1 && !roomOpenings[rooms[upRoom].roomType].down)
                            works = false;
                        if (downRoom != -1 && !roomOpenings[rooms[downRoom].roomType].up)
                            works = false;
                        break;
                    case 1://horizontal
